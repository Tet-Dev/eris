...
converterCommand.pickCommand = function pickCommand() {
    ...
    let extraFFMPEG;
    try {
        extraFFMPEG = require("ffmpeg-static")
    } catch (error) {
        
    }
    for (const command of extraFFMPEG? ["./ffmpeg", "./avconv", "ffmpeg", "avconv"].concat(extraFFMPEG) : ["./ffmpeg", "./avconv", "ffmpeg", "avconv"]) {
...
class VoiceConnection extends EventEmitter {
        ...
        */
        ...
        this.ws.on("message", (m) => {
            try {
                ...
                switch (packet.op) {
                    case VoiceOPCodes.READY: {
                        ...
                        try {
                            ...
                            this.udpSocket.once("message", (packet,rinfo) => {
                                console.log("UDP DEBUG: "+packet.toString());
                                this.emit("debug", "UDP DEBUG: "+packet.toString());
                                let localIP = "";
                                let i = 3;
                            ...
                            this.udpSocket.on("close", (err) => {
                                if (err) {
                                    this.emit("debug", "Voice UDP close: " + err);
                            ...
                            console.log("VC UDP ERROR!",error)
                        }
                ...
                console.log("Recieve Voice Message Error!",error);
    ...
    sendWS(op, data) {
        if (this.ws && this.ws.readyState === WebSocket.OPEN) {
            ...
            this.emit("debug", data);
                ...
                this.emit("debug","WS Send Error for vc!"+error);
            }
    ...
    */
    ...
    _sendPacket(packet) {
        try {
            this.udpSocket.send(packet, 0, packet.length, this.udpPort, this.udpIP,(er,bytes)=>{
                console.log("VC data"+ er+"|" +bytes)
            });
        } catch (e) {
